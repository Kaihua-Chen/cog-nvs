...
For `MegaSAM_render` on DyCheck, we stack the background and optimize the camera at test time (`tto_cam`), following a procedure similar to `render_test_tto` in `mosca_evaluate.py` from the [MoSca](https://github.com/JiahuiLei/MoSca) codebase. To reproduce this process, see `dycheck_eval_render.py`. Running this script requires [PyTorch3D](https://pytorch3d.org/); if it is difficult to install under the cognvs environment, we recommend using a separate environment.
