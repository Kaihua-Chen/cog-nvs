...
class CogNVSPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    r"""
    ...
    def prepare_latents(
        ...
        # print("prev self.vae.config.invert_scale_latents?:", self.vae.config.invert_scale_latents)

        self.vae.config.invert_scale_latents = False
        # print("self.vae.config.invert_scale_latents?:", self.vae.config.invert_scale_latents)
        ...
        # print("after scale, image_latents:", image_latents.shape, image_latents.max(), image_latents.min())
    ...
    def __call__(
        ...
        images = self.video_processor.preprocess_video(images, height=height, width=width).to(
        ...
        # latents = self.scheduler.add_noise(mage_latents, torch.randn_like(image_latents), torch.tensor(200, dtype=torch.long))
