...
from finetune.my_datasets import my_cognvs_dataset
...
class Trainer:
    ...
    def prepare_dataset(self) -> None:
        ...
        if self.args.model_type == "v2v":
            
            
            self.dataset = my_cognvs_dataset(
                base_dir_input=self.args.base_dir_input,
                base_dir_target=self.args.base_dir_target,
                mode="train",
                device=self.accelerator.device,
                frames=self.state.train_frames,
        ...
        self.eval_data_loader = torch.utils.data.DataLoader(
            self.eval_dataset,
            # collate_fn=self.collate_fn,
            batch_size=1,
            num_workers=self.args.num_workers,
    ...
    def prepare_trainable_parameters(self):
        ...
        if self.args.training_type == "sft":
            with open("layer_requires_grad_detailed.txt", "w") as f:
                for attr_name, component in vars(self.components).items():
                    if hasattr(component, "requires_grad_"):
                        # Default: freeze all components
                        component.requires_grad_(False)

                        # Special case for transformer
                        if attr_name == "transformer":
                            for name, param in component.named_parameters():
                                # Check if the parameter is in transformer_blocks.{layer}
                                if "transformer_blocks." in name:
                                    try:
                                        layer_str = name.split("transformer_blocks.")[1].split(".")[0]
    ...
    def prepare_for_training(self) -> None:
        ...
        # print("now we are in prepare_for_training!")
        ...
        # print("self.state.overwrote_max_train_steps:", self.state.overwrote_max_train_steps)
        # print("len(self.data_loader):", len(self.data_loader))
        # print("self.state.num_update_steps_per_epoch:", self.state.num_update_steps_per_epoch)
        # print("self.args.train_epochs:", self.args.train_epochs)
        # print("self.args.train_steps:", self.args.train_steps)
        # print()
    ...
    def train(self) -> None:
        ...
        for epoch in range(first_epoch, self.args.train_epochs):
            ...
            for step, batch in enumerate(self.data_loader):
                ...
                should_run_validation = self.args.do_validation and (global_step % self.args.validation_steps == 0 or global_step == 1)
    ...
    def validate(self, step: int) -> None:
        ...
        for i, batch_data in enumerate(self.eval_data_loader):

            if i > 2:
                break

            input_images, target_images, metadata = batch_data["input_images"], batch_data["target_images"], batch_data["metadata"]
            # ori_h, ori_w = int(metadata['H_orig']), int(metadata['W_orig'])
            ori_h = self.state.train_height
            ori_w = self.state.train_width


            # sequence_name = metadata["sequence"][0]
            ...
            validation_artifacts = self.validation_step({"input_pixels": input_images, "target_pixels": target_images, "global_step": step, "step": i, "ori_h": ori_h, "ori_w": ori_w}, pipe)
